...
static const char pll_default_log_file[] = "log.txt";
...
static const StaticOverlay static_overlays[] = {
    ...
    /* APU/FPU clock (APLL, DPLL, VPLL) */
    { 0xFD1A0044, 4, 0x0000003F, "CRF_PLL_STATUS" },
...
static uint32_t extract_le_subvalue(uint32_t value, hwaddr offset,
                                    unsigned size)
{
    uint32_t shifted;
    uint32_t mask;
...
static uint64_t static_overlay_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    uint32_t ret = extract_le_subvalue(ctx->value, offset, size);
    char msg_box[512];

    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] READ  %-22s @ 0x%08lX size=%u => 0x%08X\n",
             ctx->name, (unsigned long)(ctx->addr + offset), size, ret);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
                                 ...
                                 uint64_t value, unsigned size)
{
    ...
    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] WRITE %-22s @ 0x%08lX size=%u <= 0x%08lX (ignored)\n",
             ctx->name, (unsigned long)(ctx->addr + offset),
             size, (unsigned long)value);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
}
...
static void pll_bypass_run_selftest(void);

static uint64_t pgsr0_read(void *opaque, hwaddr offset, unsigned size)
{
    uint32_t ret = extract_le_subvalue(pgsr0_current, offset, size);
    char msg_box[512];

    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] READ  DDR_PGSR0              @ 0x%08lX size=%u => 0x%08X\n",
             (unsigned long)(0xFD080030 + offset), size, ret);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
                         ...
                         uint64_t value, unsigned size)
{
    ...
    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] WRITE DDR_PGSR0              @ 0x%08lX size=%u <= 0x%08lX (ignored)\n",
             (unsigned long)(0xFD080030 + offset), size,
             (unsigned long)value);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
}
...
{
    uint32_t ret = extract_le_subvalue(pir_stored, offset, size);
    char msg_box[512];

    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] READ  DDR_PIR                @ 0x%08lX size=%u => 0x%08X\n",
             (unsigned long)(0xFD080004 + offset), size, ret);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
                       ...
                       uint64_t value, unsigned size)
{
...

    {
        ...
        snprintf(msg_box, sizeof(msg_box),
                 "[PLL-BYPASS] WRITE DDR_PIR                @ 0x%08lX size=%u <= 0x%08X  PGSR0: 0x%08X -> 0x%08X\n",
                 (unsigned long)(0xFD080004 + offset), size, val,
...
static void pll_bypass_machine_init_notify(Notifier *notifier, void *data)
{
    ...
    const char *pll_selftest;
    unsigned i;

    zynqmp_log_manager_set_channel_path(ZYNQMP_LOG_CHANNEL_PLL,
                                        "QEMU_PLL_LOG_FILE",
                                        pll_default_log_file);

    custom_log(ZYNQMP_LOG_CHANNEL_PLL,
                "############################################\n");
    custom_log(ZYNQMP_LOG_CHANNEL_PLL,
                "#   PERSONALIZED QEMU -- PLL Bypass v2     #\n");
    {
        char msg_box[512];
    ...
    for (i = 0; i < NUM_STATIC_OVERLAYS; i++) {
...

        {
            ...
            snprintf(msg_box, sizeof(msg_box),
                     "[PLL-BYPASS] Mapped %-22s @ 0x%08lX  val=0x%08X\n",
    ...
    memory_region_add_subregion_overlap(sysmem, 0xFD080004,
                                        &pir_mr, 10000);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL,
                "[PLL-BYPASS] Mapped DDR_PIR                @ 0xFD080004  (stateful)\n");
                                        ...
                                        &pgsr0_mr, 10000);
    {
        ...
        snprintf(msg_box, sizeof(msg_box),
                 "[PLL-BYPASS] Mapped DDR_PGSR0              @ 0xFD080030  val=0x%08X (stateful)\n",
                 pgsr0_current);
        custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
    }

    {
        ...
        snprintf(msg_box, sizeof(msg_box),
                 "[PLL-BYPASS] Total: %u static + 2 stateful overlays active\n",
                 (unsigned)NUM_STATIC_OVERLAYS);
        custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
    }

    pll_selftest = g_getenv("QEMU_PLL_SELFTEST");
    if (pll_selftest &&
        (g_strcmp0(pll_selftest, "1") == 0 ||
         g_ascii_strcasecmp(pll_selftest, "true") == 0 ||
         g_ascii_strcasecmp(pll_selftest, "yes") == 0)) {
        custom_log(ZYNQMP_LOG_CHANNEL_PLL,
                    "[PLL-BYPASS] SELFTEST enabled via QEMU_PLL_SELFTEST\n");
        pll_bypass_run_selftest();
    }
}
...
{
    uint32_t read_val = 0;
    uint32_t pir_cmd = 0x0004FE01;
    uint32_t pgsr0_val = 0;
    char msg_box[512];

    address_space_read(&address_space_memory, 0xFD1A0044,
                       MEMTXATTRS_UNSPECIFIED,
                       (uint8_t *)&read_val, sizeof(read_val));
    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] SELFTEST read CRF_PLL_STATUS => 0x%08X\n",
             read_val);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);

    address_space_write(&address_space_memory, 0xFD080004,
                        MEMTXATTRS_UNSPECIFIED,
                        (uint8_t *)&pir_cmd, sizeof(pir_cmd));
    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] SELFTEST write DDR_PIR <= 0x%08X\n",
             pir_cmd);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);

    address_space_read(&address_space_memory, 0xFD080030,
                       MEMTXATTRS_UNSPECIFIED,
                       (uint8_t *)&pgsr0_val, sizeof(pgsr0_val));
    snprintf(msg_box, sizeof(msg_box),
             "[PLL-BYPASS] SELFTEST read DDR_PGSR0 => 0x%08X\n",
             pgsr0_val);
    custom_log(ZYNQMP_LOG_CHANNEL_PLL, msg_box);
}
